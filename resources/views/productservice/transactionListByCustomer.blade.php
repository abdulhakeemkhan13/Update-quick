    ...
    <div class="content-wrapper">
        ...
        <div class="report-content">
            <div class="report-title-section">
                ...
                <p class="date-range">
                    <span id="date-range-display">All Transactions</span>
                </p>
            </div>

            <div class="table-container p-2">
                {!! $dataTable->table(['class' => 'table transaction-list-table', 'id' => 'transaction-list-table']) !!}
            </div>
        </div>
    </div>
            ...
            <div class="modal-content">
                ...
                <div class="columns-list" id="sortable-columns">
                    ...
                    <div class="column-item" data-column="3">
                        <i class="fa fa-grip-vertical handle"></i>
                        <label class="checkbox-label">
                            <input type="checkbox" checked> Posting (Y/N)
                        </label>
                    </div>
                    ...
                    <div class="column-item" data-column="5">
                        <i class="fa fa-grip-vertical handle"></i>
                        <label class="checkbox-label">
                            <input type="checkbox" checked> Account Full Name
                        </label>
                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            function refreshData() {
                if (window.LaravelDataTables && window.LaravelDataTables["transaction-list-table"]) {
                    window.LaravelDataTables["transaction-list-table"].draw();
                } else {
            ...
            function applyHeaderFooterSettings(options) {
                ...
                if ($('.report-footer').length === 0) {
                    ...
                    const basisStr = 'Accrual Basis'; // Default for transaction report
            ...
            function updateColumnOrder() {
                ...
                localStorage.setItem('transaction-list-column-order', JSON.stringify(order));
            ...
            $('.columns-modal input[type="checkbox"]').on('change', function() {
                ...
                if (columnIndex !== undefined && window.LaravelDataTables && window.LaravelDataTables[
                        "transaction-list-table"]) {
                    try {
                        window.LaravelDataTables["transaction-list-table"].column(columnIndex).visible(
                            isVisible);
                    } catch (error) {
            ...
            $('.btn-icon[title="Print"]').on('click', function() {
                ...
                const printContent = `
                    ...
                    <html>
                    <head>
                        <title>Transaction List by Customer - Print</title>
                        <style>
            ...
            $('.btn-save').on('click', function() {
                const reportName = prompt('Enter report name:', 'Transaction List by Customer - ' + moment()
                    .format(
                        'YYYY-MM-DD'));
                if (reportName) {
                    ...
                    const settings = {
                        name: reportName,
                        customer: $('#filter-customer-modal').val(),
                        transactionType: $('#filter-transaction-type-modal').val(),
                        startDate: $('#filter-start-date-modal').val(),
                        endDate: $('#filter-end-date-modal').val(),
            ...
            function applyViewOptions() {
                ...
                if ($('#opt-compact').prop('checked')) {
                    customCSS += '.transaction-list-table th, .transaction-list-table td { padding: 8px 12px; }';
                } else {
                    customCSS += '.transaction-list-table th, .transaction-list-table td { padding: 12px 16px; }';
                }

                // Row hover effects
                if ($('#opt-hover').prop('checked')) {
                    customCSS += '.transaction-list-table tbody tr:hover { background: #f9fafb; }';
                } else {
                    customCSS += '.transaction-list-table tbody tr:hover { background: inherit; }';
                }

                // Striped rows
                if ($('#opt-striped').prop('checked')) {
                    customCSS += '.transaction-list-table tbody tr:nth-child(even) { background-color: #f8f9fa; }';
                } else {
                    customCSS += '.transaction-list-table tbody tr:nth-child(even) { background-color: inherit; }';
                }
                ...
                if ($('#opt-auto-width').prop('checked')) {
                    customCSS += '.transaction-list-table { table-layout: auto; }';
                } else {
                    customCSS += '.transaction-list-table { table-layout: fixed; }';
                }

                // Equal column widths
                if ($('#opt-equal-width').prop('checked')) {
                    customCSS += '.transaction-list-table th, .transaction-list-table td { width: 14.28%; }';
                }
                ...
                if ($('#opt-sticky-head').prop('checked')) {
                    ...
                    $('.transaction-list-table thead th').css({
                        'position': 'sticky',
                        'top': '0',
                        'z-index': '10'
                    });
            ...
            $(document).on('draw.dt', '#transaction-list-table', function() {
                if (window.reportOptions) {
                    $('.transaction-list-table tbody tr').each(function() {
                        const $row = $(this);

                        // Apply number formatting to amount column (last column)
            ...
            console.log('Transaction List by Customer Report initialized successfully');
        });
    </script>
@endpush
