...
@push('css-page')
    ...
    <style>
        ...
        /* Bar colors */
        .moneybar-section.estimate .bar { background: #21ABF6; }
        .moneybar-section.unbilled .bar { background: #9457FA; }
        .moneybar-section.overdue .bar { background: #FF8000; }
        .moneybar-section.openInvoices .bar { background: #BABEC5; }
        .moneybar-section.recentlyPaid .bar { background: #2CA01C; }
        ...
        .qbo-filters-secondary {
            ...
            gap: 8px;
            padding-top: 8px;
            margin-left: 130px; /* Align with Type filter position */
        }
        ...
        #salesTransactionsTable {
            width: 100% !important;
            min-width: 1200px; /* Ensures horizontal scrolling on smaller screens */
            margin: 0 !important;
        ...
        #salesTransactionsTable thead th {
            ...
            text-align: left;
            position: sticky;
            top: 60px; /* Sticks below main header */
            z-index: 100;
        }
        ...
        .qbo-status.overdue { color: #d93025; }
        .qbo-status.open { color: #fd7e14; }
        .qbo-status.paid { color: #2ca01c; }
        .qbo-status.draft { color: #6c757d; }
        .qbo-status.closed { color: #2ca01c; }
...
<div class="row">
    <div class="col-12">
        ...
        <div class="qbo-page-header">
            ...
            <div class="d-flex align-items-center gap-3">
                <a href="#" class="qbo-feedback-link">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" color="currentColor" width="24px" height="24px" focusable="false" aria-hidden="true">
                        <path fill="currentColor" d="M14.35 2a1 1 0 0 1 0 2H6.49a2.54 2.54 0 0 0-2.57 2.5v7A2.54 2.54 0 0 0 6.49 16h1.43a1 1 0 0 1 1 1v1.74l2.727-2.48c.184-.167.424-.26.673-.26h5.03a2.54 2.54 0 0 0 2.57-2.5v-4a1 1 0 0 1 2 0v4a4.54 4.54 0 0 1-4.57 4.5h-4.643l-4.114 3.74A1.002 1.002 0 0 1 6.92 21v-3h-.43a4.54 4.54 0 0 1-4.57-4.5v-7A4.54 4.54 0 0 1 6.49 2zm6.414.6.725.726c.79.791.79 2.074 0 2.865l-5.812 5.794c-.128.128-.29.219-.465.263l-2.9.721q-.121.03-.247.031a.998.998 0 0 1-.969-1.244l.73-2.9a1 1 0 0 1 .263-.463L17.9 2.6a2.027 2.027 0 0 1 2.864 0m-1.412 1.413-.763.724L13.7 9.612l-.255 1.015 1.016-.252 5.616-5.6V4.74z"></path>
                    </svg>
                    {{ __('Give feedback') }}
                </a>
                <button type="button" class="qbo-collapse-btn" id="moneyBarToggle" title="Toggle Money Bar">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" color="currentColor" width="24px" height="24px" focusable="false" aria-hidden="true">
                        <path fill="currentColor" d="M18.014 15.991a1 1 0 0 1-.708-.294l-5.285-5.3-5.3 5.285a.999.999 0 1 1-1.412-1.416l6.008-5.99a1 1 0 0 1 1.414 0l5.992 6.008a1 1 0 0 1-.708 1.706z"></path>
                    </svg>
                </button>
            </div>
        </div>
        ...
        <div class="qbo-money-bar" id="moneyBar">
            <section class="moneybar-group">
                ...
                <div class="moneybar-section estimate">
                    <div class="text">
                        <span class="amount">{{ Auth::user()->priceFormat($salesData['estimates']['amount'] ?? 0) }}</span>
                        <div class="label"><span>{{ $salesData['estimates']['count'] ?? 0 }} {{ __('estimates') }}</span></div>
                    </div>
                    <div class="bar" data-filter="estimate" tabindex="0" role="link" aria-label="MoneyBar filter: Estimates"></div>
                </div>

                {{-- Unbilled Income --}}
                <div class="moneybar-section unbilled">
                    <div class="text">
                        <span class="amount">{{ Auth::user()->priceFormat($salesData['unbilled']['amount'] ?? 0) }}</span>
                        <div class="label"><span>{{ __('Unbilled income') }}</span></div>
                    </div>
                    <div class="bar" data-filter="unbilledIncome" tabindex="0" role="link" aria-label="MoneyBar filter: Unbilled income"></div>
                </div>

                {{-- Overdue Invoices --}}
                <div class="moneybar-section overdue">
                    <div class="text">
                        <span class="amount">{{ Auth::user()->priceFormat($salesData['overdue']['amount'] ?? 0) }}</span>
                        <div class="label"><span>{{ $salesData['overdue']['count'] ?? 0 }} {{ __('overdue invoices') }}</span></div>
                    </div>
                    <div class="bar" data-filter="overdue" tabindex="0" role="link" aria-label="MoneyBar filter: Overdue invoices"></div>
                </div>
                ...
                <div class="moneybar-section openInvoices">
                    <div class="text">
                        ...
                        <div class="label"><span>{{ $salesData['open']['count'] ?? 0 }} {{ __('open invoices and credits') }}</span></div>
                    </div>
                    <div class="bar" data-filter="openInvoices" tabindex="0" role="link" aria-label="MoneyBar filter: Open invoices"></div>
                </div>
                ...
                <div class="moneybar-section recentlyPaid">
                    <div class="text">
                        ...
                        <div class="label"><span>{{ $salesData['paid']['count'] ?? 0 }} {{ __('recently paid') }}</span></div>
                    </div>
                    <div class="bar" data-filter="recentlyPaid" tabindex="0" role="link" aria-label="MoneyBar filter: Recently paid"></div>
                </div>
            </section>
        </div>
        ...
        <div class="qbo-info-alert" id="infoAlert">
            <svg width="26" height="24" viewBox="0 0 26 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="g5XsGF+8GhwgxGvOFOBVyg=="><path d="M0 12C0 5.37258 5.37258 0 12 0C18.6274 0 24 5.37258 24 12C24 18.6274 18.6274 24 12 24C5.37258 24 0 18.6274 0 12Z" fill="#00254A"></path><path d="M16.7438 7.3916H13.6225V10.6146H10.6562V13.1336H7.47888V16.6565H16.7438V7.3916Z" fill="#00D5B0"></path><path d="M20.9999 0C20.9999 0 20.4477 2.37016 19.4089 3.40901C18.37 4.44786 15.9999 5 15.9999 5C15.9999 5 18.37 5.55214 19.4089 6.59099C20.4477 7.62984 20.9999 10 20.9999 10C20.9999 10 21.552 7.62984 22.5909 6.59099C23.6297 5.55214 25.9999 5 25.9999 5C25.9999 5 23.6297 4.44786 22.5909 3.40901C21.552 2.37016 20.9999 0 20.9999 0Z" fill="#00D5B0"></path></svg>
            <div class="qbo-info-alert-content">
                <div class="qbo-info-alert-title">{{ __('Consider setting up late fees') }}</div>
                <div class="qbo-info-alert-text">{{ __('70% of invoices you sent in the last 12 months were paid late, or not at all. I drafted a late fee plan to encourage timely payment.') }}</div>
            </div>
            ...
            <button class="qbo-info-alert-close" onclick="document.getElementById('infoAlert').style.display='none'">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" color="currentColor" width="20px" height="20px" focusable="false" aria-hidden="true" class=""><path fill="currentColor" d="m13.432 11.984 5.3-5.285a1 1 0 1 0-1.412-1.416l-5.3 5.285-5.285-5.3A1 1 0 1 0 5.319 6.68l5.285 5.3L5.3 17.265a1 1 0 1 0 1.412 1.416l5.3-5.285L17.3 18.7a1 1 0 1 0 1.416-1.412l-5.284-5.304Z"></path></svg>
            </button>
        </div>
            ...
            <div class="qbo-filters-primary">
                ...
                <div class="dropdown" style="margin-left: auto;">
                    <button type="button" class="qbo-new-btn" data-bs-toggle="dropdown">
                        {{ __('New transaction') }}
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" color="currentColor" width="16px" height="16px" focusable="false" aria-hidden="true" class="dropdown-arrow">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end qbo-new-dropdown">
                        <li><a class="dropdown-item" href="{{ route('invoice.create', 0) }}">{{ __('Invoice') }}</a></li>
                        <li><a class="dropdown-item" href="{{ route('invoice.create', 0) }}">{{ __('Import invoices') }}</a></li>
                        <li><a class="dropdown-item" href="{{ route('receive-payment.create') }}">{{ __('Payment') }}</a></li>
                        <li><a class="dropdown-item" href="{{ route('proposal.create', 0) }}">{{ __('Estimate') }}</a></li>
                        <li><a class="dropdown-item" href="#" onclick="showComingSoon(); return false;">{{ __('Payment Link') }}</a></li>
                        <li><a class="dropdown-item" href="{{ route('sales-receipt.create') }}">{{ __('Sales Receipt') }}</a></li>
                        <li><a class="dropdown-item" href="{{ route('creditmemo.create', 0) }}">{{ __('Credit Memo') }}</a></li>
                        <li><a class="dropdown-item" href="{{ route('refund-receipt.create') }}">{{ __('Refund Receipt') }}</a></li>
                        <li><a class="dropdown-item openDelayedCreditModal" href="#" data-url="{{ route('delayed-credit.create') }}">{{ __('Delayed Credit') }}</a></li>
                        <li><a class="dropdown-item openDelayedChargeModal" href="#" data-url="{{ route('delayed-charge.create') }}">{{ __('Delayed Charge') }}</a></li>
            ...
            <div class="qbo-filters-secondary">
                ...
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu qbo-secondary-dropdown">
                        <li><a class="dropdown-item" href="#" onclick="setFilter('status', 'all'); return false;">{{ __('All') }}</a></li>
                        <li><a class="dropdown-item" href="#" onclick="setFilter('status', 'open'); return false;">{{ __('Open') }}</a></li>
                        <li><a class="dropdown-item" href="#" onclick="setFilter('status', 'overdue'); return false;">{{ __('Overdue') }}</a></li>
                        <li><a class="dropdown-item" href="#" onclick="setFilter('status', 'paid'); return false;">{{ __('Paid') }}</a></li>
                        <li><a class="dropdown-item" href="#" onclick="setFilter('status', 'pending'); return false;">{{ __('Pending') }}</a></li>
                        <li><a class="dropdown-item" href="#" onclick="setFilter('status', 'accepted'); return false;">{{ __('Accepted') }}</a></li>
                        <li><a class="dropdown-item" href="#" onclick="setFilter('status', 'closed'); return false;">{{ __('Closed') }}</a></li>
                        <li><a class="dropdown-item" href="#" onclick="setFilter('status', 'converted'); return false;">{{ __('Converted') }}</a></li>
                        <li><a class="dropdown-item" href="#" onclick="setFilter('status', 'declined'); return false;">{{ __('Declined') }}</a></li>
                        <li><a class="dropdown-item" href="#" onclick="setFilter('status', 'expired'); return false;">{{ __('Expired') }}</a></li>
                        <li><a class="dropdown-item" href="#" onclick="setFilter('status', 'voided'); return false;">{{ __('Voided') }}</a></li>
                    </ul>
                </div>
                ...
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu qbo-secondary-dropdown">
                        <li><a class="dropdown-item" href="#" onclick="setFilter('delivery', 'any'); return false;">{{ __('Any') }}</a></li>
                        <li><a class="dropdown-item" href="#" onclick="setFilter('delivery', 'send_later'); return false;">{{ __('Send later') }}</a></li>
                        <li><a class="dropdown-item" href="#" onclick="setFilter('delivery', 'print_later'); return false;">{{ __('Print later') }}</a></li>
                    </ul>
                </div>
                ...
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu qbo-secondary-dropdown">
                        <li><a class="dropdown-item" href="#" onclick="setFilter('errors', 'none'); return false;">{{ __('None') }}</a></li>
                        <li><a class="dropdown-item" href="#" onclick="setFilter('errors', 'delivery'); return false;">{{ __('Delivery errors') }}</a></li>
                        <li><a class="dropdown-item" href="#" onclick="setFilter('errors', 'payment'); return false;">{{ __('Payment processing errors') }}</a></li>
                    </ul>
                </div>
            </div>
        ...
        <div class="qbo-table-wrapper">
            ...
            <table class="table" id="salesTransactionsTable">
                    ...
                    <tr>
                        ...
                        <th>
                            <div class="qbo-header-actions">
                                {{ __('ACTION') }}
                                <button class="qbo-header-action-btn" title="{{ __('Print') }}"><i class="ti ti-printer"></i></button>
                                <button class="qbo-header-action-btn" title="{{ __('Export') }}"><i class="ti ti-download"></i></button>
                                <button class="qbo-header-action-btn" title="{{ __('More') }}"><i class="ti ti-dots-vertical"></i></button>
                                <button class="qbo-header-action-btn" title="{{ __('Settings') }}"><i class="ti ti-settings"></i></button>
                            </div>
                        </th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        <td><input type="checkbox" class="form-check-input row-checkbox" value="{{ $txn['id'] ?? '' }}"></td>
                        ...
                        <td>
                            @if(!empty($txn['view_url']))
                                <a href="{{ $txn['view_url'] }}" class="qbo-action-link">{{ __('View/Edit') }}</a>
                            @endif
                        </td>
                    </tr>
                    @empty
...
<script>
    $(document).ready(function() {
        ...
        var rowCount = table.find('tbody tr').not('.qbo-total-row').length;
        
        if (rowCount > 1 || (rowCount === 1 && !table.find('tbody tr td[colspan="9"]').length)) {
            table.DataTable({
                order: [[1, 'desc']],
                pageLength: 25,
                dom: 'rtip',
